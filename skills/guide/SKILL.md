...
- The pages, and the captures they cite, close **`by artifact`**.
...
A guide phase's diff is `.md` and `.png`, so **CHECK's machine stage goes green for free** —
that green is about code this phase never touched. No screen is modified either, so
`references/uiaudit.md` **in the loop skill** does not fire; the pages are the artifact instead.

## Before anything else

Five phrasings will arrive ahead of these instructions. Refuse each by name:
...
| **Devices** | desktop · mobile · both | ask — it sets the capture width |
